...
#sql_file = r"C:\Users\spachava\Downloads\Export_master_extra_doc\PostgreSQL_postGIS_dump\2-Database_data\dump_datastorage_onlyData.sql"
sql_file = 'workload1.sql'
queries = read_queries_from_file(sql_file)

# Run battery report before starting the workload
start_battery_report = "battery_rep_start.html"
...
for idx, query in enumerate(queries):
    ...
    query_result = cur.fetchall()  # Fetch the results of the query (commit for UPDATE workload)
    results.append((idx + 1, query_result))  # Use query number (index + 1) (commit for UPDATE workload)
    ...
    if idx == len(queries) - 1:
        end_battery_report = "battery_rep_end.html"
...
# Export query results to CSV (entire class commit for UPDATE workload)
with open('query_results_BIndex_WL2_1.csv', 'w', newline='') as results_file:
...
with open('exe_times_AIndex_WL6_2GB_1.csv', 'w', newline='') as times_file:
