    ...
    def select_candidate_indices(self):
        ...
        for column, data in self.frequencies.items():
            ...
            if frequency >= 3 and '*' not in column and 'COUNT(*)' not in column:  
                candidate_indices.append((column, frequency, queries))  #in queries place 'len(set(queries))'
...
class CardinalityChecker:
    ...
    def check_cardinality(self, candidate_indices):
        ...
        for column, frequency, queries in candidate_indices:
            column_name = column.split(' ->> ')[-1].strip("'")
            cardinality_value = self.cardinality.get(column_name, None)

            if cardinality_value is not None:
                CardCount.append((column, frequency, cardinality_value, queries))
            else:
                CardCount.append((column, frequency, 'Cardinality not found', queries))
...
class ProcessIndexSelector:
    ...
    def select_process_indices(self):
        process_indices = []
        for column, frequency, count, queries in self.counts:
            if isinstance(count, int) and count >= 5:
                process_indices.append((column, frequency, count, queries))
...
def main(file_path, cardinality_csv_path):
    ...
    for column, frequency, queries in candidate_indices:
        print(f"{column} is used by {len(set(queries))} queries")
