...
start_battery_report = "battery_rep_start_BIndex_WL5_6.html"
...
for idx, query in enumerate(queries):
    ...
    # Extract the actual execution time from the explain analyze result
    execution_time = None
    for row in explain_result:
        if 'Execution Time' in row[0]:
            execution_time = float(re.search(r'Execution Time: ([0-9.]+) ms', row[0]).group(1))
            break
    ...
    if idx == len(queries) - 1:
        end_battery_report = "battery_rep_end_BIndex_WL5_6.html"
...
with open('exe_times_BIndex_WL4_6.csv', 'w', newline='') as times_file:
