...
df_bindex = pd.read_csv(r"C:\Users\spachava\Downloads\M2_Stage_Energy_Opt_Code_Final_Files\Plot_Graph_Codes\Workload1\exe_times_WL1_BIndex.csv", encoding='utf-8').iloc[:-2]
df_aindex = pd.read_csv(r"C:\Users\spachava\Downloads\M2_Stage_Energy_Opt_Code_Final_Files\Plot_Graph_Codes\Workload1\exe_times_WL1_AIndex.csv", encoding='utf-8').iloc[:-2]
...
plt.bar(bar1, df['Execution_Time_BIndex(ms)'], width=bar_width, label='Before ndex(ms)')

# Plot Execution Time AIndex
plt.bar(bar2, df['Execution_Time_AIndex(ms)'], width=bar_width, label='After Index(ms)')

# Add titles and labels
plt.title('Estimated Execution Time Comparison', fontsize = 20)
